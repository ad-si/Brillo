...
testLosslessAnimation outputDir = do
  ...
  sequence
    [ runTest "Export animation frames" $ do
        ...
        allExist <- mapM (\i -> doesFileExist (outputDir </> printf "lossless_anim%d.webp" (i :: Int))) [1..5]
    ...
    , runTest "Export rotating square animation" $ do
        ...
        exportPicturesToWebP testSize white filePattern animation timePoints
        allExist <- mapM (\i -> doesFileExist (outputDir </> printf "lossless_rotate%d.webp" (i :: Int))) [1..5]
    ...
    , runTest "Export color changing animation" $ do
        ...
        exportPicturesToWebP testSize white filePattern animation timePoints
        allExist <- mapM (\i -> doesFileExist (outputDir </> printf "lossless_color%d.webp" (i :: Int))) [1..5]
...
testLossyAnimation outputDir = do
  ...
  sequence
    [ runTest "Export lossy animation frames" $ do
        ...
        allExist <- mapM (\i -> doesFileExist (outputDir </> printf "lossy_anim%d.webp" (i :: Int))) [1..3]
    ...
    , runTest "Export lossy animation with different qualities" $ do
        ...
        hqExist <- mapM (\i -> doesFileExist (outputDir </> printf "lossy_hq%d.webp" (i :: Int))) [1..3]
        lqExist <- mapM (\i -> doesFileExist (outputDir </> printf "lossy_lq%d.webp" (i :: Int))) [1..3]
...
testFileValidity outputDir = do
  ...
  sequence
    ...
    , runTest "File size is reasonable" $ do
        ...
        assertBool ("File size should be reasonable (got " ++ show size ++ " bytes)")
